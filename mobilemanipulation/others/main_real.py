...
def discrete_dqn_grasping(args):
    ...
    if REAL_ENV:
        if ROTATION:
            discrete_dimensions = [15, 15, 10]
            discrete_dimension = 15 * 15 * 10
            discretizer = Discretizer(discrete_dimensions, [-0.5, -0.5, -0.5], [0.5, 0.5, 0.5])
    ...
    if args.load_data:
        ...
        train_data_files = glob.glob('others/logs/static__11072021-17-50-47/train_buffer.npy')
...
def real_soft_q_grasping(args):
    ...
    # validation_buffer = ReplayBuffer(size=num_samples_total+1000, observation_shape=(image_size, image_size, 3), 
    #                                  action_dim=1, raw_action_dim=(len(discrete_dimensions),))
    validation_buffer = ReplayBuffer(size=num_samples_total, observation_shape=(image_size, image_size, 3),
                                action_dim=1, raw_action_dim=(len(discrete_dimensions),))
